...
# gstack-plugin 경로 탐색
_GSTACK_BIN=""
for _P in $(find ~/.claude/plugins/cache -path "*/gstack-plugin/*/bin" -type d 2>/dev/null) ~/.claude/skills/gstack/bin; do
...
[ -z "$B" ] && [ -n "$_GSTACK_BIN" ] && B="$_GSTACK_BIN"/../browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
