...
```bash
[ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "release-quality-gate-1-$(date -u +%Y%m%d)" "release-quality-gate" "{success|error}" "$(( $([ -n "$_EMIT" ] && bash "$_EMIT" now_ms || echo 0) - {agent_start_ms} ))" "출시 준비 검토 완료"
