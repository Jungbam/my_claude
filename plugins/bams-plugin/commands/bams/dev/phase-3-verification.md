...
[ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "qa-strategy-8-$(date -u +%Y%m%d)" "qa-strategy" "success" {duration_ms} "3관점 리뷰 완료"
[ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "product-analytics-8-$(date -u +%Y%m%d)" "product-analytics" "success" {duration_ms} "성과 평가 완료"
```
