...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "pipeline-orchestrator-5-{N}-$(date -u +%Y%m%d)" "pipeline-orchestrator" "claude-fable-5" "Step 5: 배치 {N} 라우팅 조언 요청"
```
