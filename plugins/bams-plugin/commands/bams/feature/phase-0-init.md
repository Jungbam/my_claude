...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "resource-optimizer-0-$(date -u +%Y%m%d)" "resource-optimizer" "sonnet" "Step 0: feature 파이프라인 전략 수립"
```

Task tool, subagent_type: **"bams-plugin:resource-optimizer"**, model: **"sonnet"** — 메인이 직접 호출:
