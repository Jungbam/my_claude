...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "pipeline-orchestrator-5-$(date -u +%Y%m%d)" "pipeline-orchestrator" "success" {duration_ms} "Step 5 Advisor 응답 수신"
```
...
반환 후 agent_end emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "qa-strategy-5-$(date -u +%Y%m%d)" "qa-strategy" "success" {duration_ms} "Step 5 완료: 5관점 코드 리뷰 완료"
```
...
반환 후 agent_end emit + Advisor Response 파싱 + CHAIN_VIOLATION 체크:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "pipeline-orchestrator-678-$(date -u +%Y%m%d)" "pipeline-orchestrator" "success" {duration_ms} "Step 6-7-8 Advisor 응답 수신"
```
...
[ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "qa-strategy-678-$(date -u +%Y%m%d)" "qa-strategy" "success" {duration_ms} "QA + 보안 감사 완료"
[ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "product-analytics-678-$(date -u +%Y%m%d)" "product-analytics" "success" {duration_ms} "성능 베이스라인 완료"
```
...
반환 후 agent_end emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "platform-devops-9-$(date -u +%Y%m%d)" "platform-devops" "success" {duration_ms} "Step 9 CI/CD 완료"
```
...
반환 후 agent_end emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "cross-department-coordinator-9-$(date -u +%Y%m%d)" "cross-department-coordinator" "success" {duration_ms} "Step 9 검증→배포 핸드오프 완료"
```
