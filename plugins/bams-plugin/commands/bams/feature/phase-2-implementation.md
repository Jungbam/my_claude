...
Bash로 다음을 실행합니다:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" step_start "{slug}" 4 "멀티에이전트 개발" "Phase 2: 구현"
```
...
### Step 4-a. pipeline-orchestrator 조언 요청 (배치 {N} 라우팅)

각 배치 호출 전, Bash로 agent_start emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "pipeline-orchestrator-4-{N}-$(date -u +%Y%m%d)" "pipeline-orchestrator" "claude-fable-5" "Step 4: 배치 {N} 라우팅 조언 요청"
```
...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "pipeline-orchestrator-4-{N}-$(date -u +%Y%m%d)" "pipeline-orchestrator" "success" {duration_ms} "Step 4 배치 {N} Advisor 응답 수신"
```

### Step 4-b. 메인이 권고된 부서장들 병렬 직접 spawn
...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "{dept}-4-{N}-$(date -u +%Y%m%d)" "{dept}" "success" {duration_ms} "Step 4 배치 {N} 부서장 완료"
```
...
- **검증 진행 (Recommended)**
- **구현까지만** — `status: paused_at_step_4` 기록 후 종료.
