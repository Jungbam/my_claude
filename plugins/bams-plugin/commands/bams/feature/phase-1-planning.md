...
1. Task tool, subagent_type: **"bams-plugin:product-strategy"**, model: **"opus"** — 기능 명세:
> ```
...
2. Task tool, subagent_type: **"bams-plugin:frontend-engineering"**, model: **"opus"** — FE 기술 설계:
> ```
...
3. Task tool, subagent_type: **"bams-plugin:backend-engineering"**, model: **"opus"** — BE 기술 설계:
> ```
...
Bash로 agent_start emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "pipeline-orchestrator-handoff1-$(date -u +%Y%m%d)" "pipeline-orchestrator" "opus" "기획→구현 게이트 판정 조언"
```
