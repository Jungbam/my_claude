...
### Step 12. 문서 갱신 (기획부장 위임) — Ship 직후 시작

**최적화**: 문서 갱신은 Deploy(Step 11)와 독립적이므로, **Step 10(Ship) 완료 직후 백그라운드로 시작**합니다.
Step 11(Deploy) 선택 시 문서 갱신과 배포가 병렬로 진행됩니다.

**스킬 미설치 시**: Step 12 `skipped`.

Bash로 다음을 실행합니다:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" step_start "{slug}" 12 "문서 갱신" "Phase 5: 마무리"
```
...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "product-strategy-12-$(date -u +%Y%m%d)" "product-strategy" "claude-fable-5" "Step 12: 문서 갱신"
```

Task tool, subagent_type: **"bams-plugin:product-strategy"** — 메인이 직접 호출:

> **Phase 5 Step 12 — 문서 갱신**
>
> ```
...
### Step 13. 스프린트 종료 (project-governance 위임)

Bash로 다음을 실행합니다:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" step_start "{slug}" 13 "스프린트 종료" "Phase 5: 마무리"
```
...
Bash로 agent_start emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "product-strategy-13-$(date -u +%Y%m%d)" "product-strategy" "claude-fable-5" "Step 13: 스프린트 종료"
```

Task tool, subagent_type: **"bams-plugin:product-strategy"** — 메인이 직접 호출:

> **Phase 5 Step 13 — 스프린트 종료**
>
> ```
...
### Step 14. 자동 강제 회고 (executive-reporter + 부서장들)

**이 단계는 건너뛸 수 없습니다. 자동으로 강제 실행됩니다.**

Bash로 다음을 실행합니다:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" step_start "{slug}" 14 "자동 강제 회고" "Phase 5: 마무리"
```

**루프 B — Advisor 조언 후 메인이 executive-reporter + 참여 부서장들 병렬 직접 spawn.** 이 단계는 스킵할 수 없습니다.

### Step 14-a. pipeline-orchestrator 조언 요청 (Advisor)

Bash로 agent_start emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "pipeline-orchestrator-14-$(date -u +%Y%m%d)" "pipeline-orchestrator" "claude-fable-5" "Step 14: 회고 조언 요청"
```

Task tool, subagent_type: **"bams-plugin:pipeline-orchestrator"** — **조언자 모드**:

> **Phase 5 Step 14 Advisor 호출 — 자동 강제 회고 라우팅 권고**
>
> **컨텍스트:**
> ```
> phase: 5-retro
...
카운터(`{completed}`/`{failed}`/`{skipped}`)는 셸 변수가 아니라 **대화 컨텍스트에서 추적**한 값을 리터럴로 직접 기입한다(harness의 각 Bash 호출은 새 셸이라 변수가 유지되지 않는다).
...
```
(`{status}`는 `completed` / `paused` / `failed` 중 하나, `{total}`은 15 — Step 0~14 총 15개)

Step 14에서 이미 자동 강제 회고(KPT 수집 + 합의 도출 + 피드백 반영)를 수행했으므로, completion-protocol Step 4.95의 `/bams:retro` 제안 시 중복 실행하지 않는다.

**`references/completion-protocol.md` 참조.** 표준 프로토콜을 따릅니다.

이 파이프라인의 Learnings 카테고리:
1. `pattern:` — 새로 도입한 패턴/라이브러리
2. `convention:` — 리뷰(Step 5)에서 발견된 코드 컨벤션
3. `vulnerable:` — 보안 감사(Step 8)/리뷰에서 반복 지적된 영역
4. `perf-baseline:` — 벤치마크(Step 7) 수치
