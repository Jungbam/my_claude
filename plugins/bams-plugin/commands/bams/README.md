...
## QA 관련 커맨드 구분
- `/bams:qa` — 브라우저 기반 QA 테스트 워크플로우 (자동화 테스트 계획 + 브라우저 검증)
- `/bams:qc` — QA부서 허브 (qa-strategy 부서장 + 4개 에이전트 선택)
- `/bams:verify` — CI/CD 프리플라이트 (빌드, 린트, 타입체크, 테스트 실행)
...
| `/bams:review` | 5개 qa-strategy 에이전트 동시 실행 (`--aspect` 지원) |
| `/bams:deep-review` | 5관점 ∥ 구조적리뷰 ∥ Codex 최대 7개 동시 실행 (`--aspect` 지원) |
...
각 파이프라인 실행 시 `~/.bams/artifacts/pipeline/` 에 진행 상태가 기록됩니다.
