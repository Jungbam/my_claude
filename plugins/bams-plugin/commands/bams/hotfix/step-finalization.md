...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "pipeline-orchestrator-6-$(date -u +%Y%m%d)" "pipeline-orchestrator" "opus" "Step 4.5: 에이전트 개선점 분석 조언"
```

서브에이전트 실행 (Agent tool, subagent_type: **"bams-plugin:pipeline-orchestrator"**, model: **"opus"**, **조언자 모드**):
...
## 마무리: 자동 회고 (축소판, 2단 위임 — 루프 B 동적 부서장)

> `_shared_common.md` §부록 루프 B 준수. Advisor가 참여 부서장 목록을 동적 반환하고, 메인이 파싱 후 해당 부서장을 직접 병렬 spawn합니다.

**Phase 1: Advisor 호출 (pipeline-orchestrator, 조언자 모드)**

Bash로 agent_start를 emit합니다:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "pipeline-orchestrator-7-$(date -u +%Y%m%d)" "pipeline-orchestrator" "sonnet" "마무리: 핫픽스 축소 회고 조언"
```

서브에이전트 실행 (Agent tool, subagent_type: **"bams-plugin:pipeline-orchestrator"**, model: **"sonnet"**, **조언자 모드**):

> **조언자 요청 — 핫픽스 축소 회고 동적 부서장 결정**
>
> **컨텍스트:** triage `.crew/artifacts/hotfix/{slug}-triage.md`, pipeline_type: hotfix
>
> **반환 요구 (Advisor Response 계약):**
> 1. 회고 기록을 담당할 1차 에이전트(기본: executive-reporter)
> 2. 핫픽스 영역에 따라 추가로 참여할 부서장 목록(동적) — 예: backend/frontend/platform-devops 등 근본원인 부서장
> 3. 각 대상별 task_description 초안
> 4. board.md / DB Done 이동 대상 태스크 ID 후보
> 5. spawn 순서(병렬 가능 여부)
>
> spawn 지시 금지. 메인이 결과를 받아 직접 병렬/순차 호출합니다.

Bash로 agent_end를 emit합니다:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "pipeline-orchestrator-7-$(date -u +%Y%m%d)" "pipeline-orchestrator" "success" {duration_ms} "마무리 조언 완료"
```

**Phase 2: 메인이 직접 부서장 병렬 spawn**

Advisor Response를 파싱한 후, 메인이 다음을 수행합니다:

1. 먼저 executive-reporter(+ 추가 부서장 목록) 전체의 `agent_start`를 Bash로 모두 emit
2. Agent tool로 **병렬 호출** (각 subagent_type = Advisor가 지정한 값):
   - **executive-reporter**: 아래 3개 학습 카테고리 기록
     - `hotfix:` — 근본 원인 + 영향 범위 요약
     - `vulnerable:` — 같은 영역 반복 버그 감지 시 경고 수준 상향
     - `regression-test:` — 추가 회귀 테스트 경로
   - **추가 부서장(동적)**: Advisor가 반환한 각 부서장에게 자기 부서 관점 교훈 1~3줄 기록 위임
