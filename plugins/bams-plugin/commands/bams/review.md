...
## Phase 2: 5관점 병렬 리뷰 (QA부장 위임)

pipeline-orchestrator에게 검증을 시작하도록 지시합니다.
...
>
> **수행할 작업:**
> qa-strategy(QA부장)에게 5관점 병렬 리뷰를 위임합니다:
>
> ```
...
> ```
>
> qa-strategy는 내부적으로 5개 관점 에이전트(또는 automation-qa, defect-triage 등 적절한 하위 에이전트)를 병렬로 활용하여 리뷰를 진행합니다.
>
...
## Phase 4: 릴리스 품질 게이트 (QA부장 위임)

pipeline-orchestrator에게 릴리스 품질 게이트 실행을 지시합니다.

서브에이전트 실행 (Task tool, subagent_type: **"bams-plugin:pipeline-orchestrator"**, model: **"sonnet"**):

> **검증 Phase 4 — 릴리스 품질 게이트**
>
> **위임 메시지:**
> ```
> phase: 4
...
> pipeline_type: review
> context:
>   review_result: [Phase 3의 종합 결과 — 심각도별 이슈 수, 상세 이슈 목록]
>   changed_files: [파일 목록]
>   config: .crew/config.md
> ```
>
> **수행할 작업:**
> qa-strategy(QA부장)에게 release-quality-gate 실행을 위임합니다:
>
> ```
> task_description: "코드 리뷰 결과를 종합 판정하라"
> input_artifacts:
>   - review_result: [Phase 3 종합 결과]
>   - .crew/config.md
> expected_output:
...
> ```
>
> qa-strategy는 내부적으로 release-quality-gate 에이전트를 활용합니다.
>
