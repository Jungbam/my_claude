---
description: 5관점 병렬 리뷰 + 릴리스 게이트 판정 (--aspect 지원: spec/functional/performance/code/uiux/all). 구조적 리뷰·Codex 포함 심층 조사는 `/bams:deep-review`
argument-hint: [파일, 디렉토리, 또는 "pr"] [--aspect <spec|functional|performance|code|uiux|all>[,...]]
---
...
- `ASPECT_LIST == ["code"]`(인자 없음)이면 본 섹션 이후 아무 것도 변경되지 않는다 — **이하 Phase 1~8을 기존과 완전히 동일하게 실행**(하위 호환).
- 그 외의 경우, 이후 Phase 2/4에서 `ASPECT_LIST`를 참조한다.
...
> 본 커맨드와 deep-review의 차이는 `references/multi-perspective-review.md` §역할 구분 참조.
...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "pipeline-orchestrator-2-$(date -u +%Y%m%d)" "pipeline-orchestrator" "success" "$(( $([ -n "$_EMIT" ] && bash "$_EMIT" now_ms || echo 0) - {agent_start_ms} ))" "Phase 2 Advisor 응답 수신"
```
...
4. 심각도별 총 이슈 수 집계
5. (다중 aspect 시) `references/multi-perspective-review.md` §종합/중복 제거 규칙 6번 — aspect 태그 부여
...
Bash로 agent_start emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "qa-strategy-4-$(date -u +%Y%m%d)" "qa-strategy" "claude-opus-4-8" "Phase 4: 릴리스 품질 게이트"
```
...
> quality_criteria:
>   판정 기준: **`references/issue-severity.md` §Release Gate 임계값 참조.**
>   차이점: 본 파이프라인 N값 = **2** (기본값 — SSOT §파이프라인별 Override 확인).
>   (다중 aspect 시) `references/issue-severity.md` §aspect별 Override와 worst-case 원칙을 함께 적용.
> ```
>
...
반환 후 agent_end emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_end "{slug}" "qa-strategy-4-$(date -u +%Y%m%d)" "qa-strategy" "success" "$(( $([ -n "$_EMIT" ] && bash "$_EMIT" now_ms || echo 0) - {agent_start_ms} ))" "릴리스 게이트 판정 완료"
```
