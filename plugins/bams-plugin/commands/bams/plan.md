...
[ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "frontend-engineering-2-$(date -u +%Y%m%d)" "frontend-engineering" "claude-opus-4-8" "Phase 2: FE 기술 설계"
...
# 3트랙 중 에러가 하나라도 있으면 이 step은 실패로 카운트, 모두 성공이면 완료로 카운트
