...
```bash
_BP=$(find ~/.claude/plugins/cache -path "*/bams-plugin/*/references/best-practices/qa-strategy.md" 2>/dev/null | head -1)
[ -z "$_BP" ] && _BP=$(find . -path "*/bams-plugin/references/best-practices/qa-strategy.md" 2>/dev/null | head -1)
[ -n "$_BP" ] && echo "참조: $_BP"
```
