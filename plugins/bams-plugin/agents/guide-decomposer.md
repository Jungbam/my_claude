...
- `sandbox`: `"read-only"` (가이드 정적 분석·토큰 추출 전용)  ·  `cwd`: 대상 프로젝트 루트 절대경로  ·  `approval-policy`: `"never"` (비대화형 파이프라인)
...
- 각 파일의 항목 수를 result_summary에 포함 (예: `"components: 12, tokens: 34, colors: 8 (via gpt-5.6-luna)"`).
