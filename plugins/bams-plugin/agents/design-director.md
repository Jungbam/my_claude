...
2. **2026 트렌드 전략 수립 (trend_strategy)**: AI-native UX, 적응형 인터페이스, Bento Grid 레이아웃, Glassmorphism 2.0, 다크 모드 퍼스트 접근, 모션 중심 스토리텔링 등 2026 디자인 트렌드를 분석하고 제품에 적합한 트렌드를 선별하여 전략에 반영한다.
...
- AI-native UX: 생성형 UI, 적응형 레이아웃, 개인화 인터페이스 패턴 우선 검토
- 모션 퍼스트: 정적 와이어프레임보다 모션 프로토타입을 먼저 검토하는 방식 채택
