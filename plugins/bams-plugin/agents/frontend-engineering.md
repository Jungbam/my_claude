...
위임 메시지 수신 시 반복 도메인 여부를 먼저 판정한다:

1. 파이프라인 slug/description에서 도메인 키워드 1~2개 추출 (예: `결제`, `인증`)
2. 최근 7일 내 동일 키워드 파이프라인 카운트
...
위임 메시지 수신 후, **구현에 착수하기 전에 반드시** 다음 3줄을 출력한다. 이 출력이 없으면 구현을 시작하지 않는다.
