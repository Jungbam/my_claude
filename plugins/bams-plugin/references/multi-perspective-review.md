...
| `uiux` | 시각 충실도·접근성·사용성 | design-director (`gpt-5.3-codex`) | visual-fidelity-verifier, accessibility-auditor | 24 |
...
| uiux | 시각 충실도·접근성·사용성 | 스크린샷 경로 또는 대상 URL(없으면 정적 diff만 수행) | uiux_review | WCAG AA 위반은 Critical(issue-severity.md §aspect별 Override), 모델은 `gpt-5.3-codex` 고정 |
